...
| `plot`    | path-only | `histogram`, `line` (`line(ys)` over the index, or `line(xs, ys)` with a deterministic numeric x-axis — e.g. `plot::line(bit_counts, errors)`), `scatter`, `heatmap`, `corr`, `fan` (quantile-band cone of a path), `explain`, `value` — write the path (`plot::fan(...)`); charts are pushed into the document at their statement's position |
